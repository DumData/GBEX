...
def url_gen(model):
	...
	return [
		...
		path(f'{mnl}/batch/<pk>', GBEXBatchList.as_view(model=model.batchmodel), name=f'batch_{mnl}'),
	]
...
] + list(chain.from_iterable([url_gen(model) for model in apps.get_app_config('GBEX_app').get_models() if hasattr(model, "GBEX_Page")])) + [ #hasattr includes actual GBEXpages and also batches, which wont be shown due to tags.py
	path(f'{model.__name__}/autocomplete/', GBEXAutocomplete.as_view(model=model, search_fields=["name"], create_field='name'), name=f'{model.__name__}-autocomplete') for model in apps.get_app_config('GBEX_app').get_models() if getattr(model, "GBEX_Option", False)
]